...
static void log_callback_myfile(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    int needed = vsnprintf(stack_buffer, sizeof(stack_buffer), fmt, vl);
    
    // Check if the stack buffer was large enough
    if (needed < 0) {
        // vsnprintf encoding error, handle accordingly
        return;
    } else if (needed < sizeof(stack_buffer)) {
        // The message fit into the stack buffer, use it
        
        if (g_log_message_callback) {
            g_log_message_callback(level, stack_buffer);
        }
    } else {
        // Allocate a larger buffer based on needed size
        char *heap_buffer = malloc(needed + 1); // +1 for null terminator
        if (!heap_buffer) {
            // Memory allocation failed, handle accordingly
            return;
        }

        // Print the message to the heap buffer
        vsnprintf(heap_buffer, needed + 1, fmt, vl);

        if (g_log_message_callback) {
            g_log_message_callback(level, heap_buffer);
        }
...
static void *ffmpeg_main_thread(void *main_args_void_ptr)
{
    ...
    int id = main_args->id;
    log_message_callback_t log_callback = main_args->log_callback;
    ...
    if (log_callback != NULL) {
        g_log_message_callback = log_callback;
...
DLL_EXPORT int ffmpeg_start(int argc, char **argv, int id, log_message_callback_t log_callback)
{
    ...
    main_args->argv = argv;
    main_args->id = id;
    main_args->log_callback = log_callback;
...
static inline void ltrim_chenfa(char *pStr)  
{  
    char *pTmp = pStr;
...
static inline void rtrim_chenfa(char *pStr)  
{  
...
DLL_EXPORT int ffmpeg_start_str(const char* oldP,int id,log_message_callback_t log_callback)
{
    ...
    MainArgs *main_args = av_malloc(sizeof(MainArgs));
    main_args->argc = arg_count;
    main_args->argv = new_argv_out;
    main_args->id = id;
    main_args->log_callback = log_callback;
